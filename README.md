...
# FiveM React Vite Boilerplate

Boilerplate para criar interfaces **NUI** no FiveM usando **React + Vite + TypeScript**, já pronto para comunicação entre **Lua ↔ React**.
...
```bash
unzip fivem-react-vite-boilerplate.zip -d resources/
```

### 2. Instalar dependências do frontend

```bash
cd resources/fivem-react-vite-boilerplate/web
npm install
```
...
- Coloque a pasta `fivem-react-vite-boilerplate` dentro de `resources/`.
- No `server.cfg`, adicione:

```cfg
start fivem-react-vite-boilerplate
```
...
## Instalando TailwindCSS 4 no projeto

1. Instale TailwindCSS e dependências:

```bash
npm install tailwindcss @tailwindcss/vite
```

2. Crie o arquivo de configuração do Tailwind:

```bash
npx tailwindcss init -p
```

3. Configure o Tailwind no `tailwind.config.js`:
...
4. Importe Tailwind no CSS principal (`src/index.css`):

```css
@import "tailwindcss";
```

5. Importe no `vite.config.ts`:
...
6. Use classes Tailwind nos componentes React.
